...
const NewsTicker: React.FC<NewsTickerProps> = ({
  ...
  const {
    lastUpdate
  } = useSupabaseRealtime('news_ticker', {
    showToasts: false
  });
    ...
    const fetchMessages = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('news_ticker').select('*').eq('is_active', true).order('created_at', {
          ascending: false
        });
        if (error) {
  ...
  return <div className={`bg-primary/10 text-primary py-2 px-4 ${className}`}>
      <div className="container max-w-7xl flex items-center mx-[240px]">
