...
export default function KingdomsPage() {
  ...
  const handleEditClick = () => {
    ...
    setNewName(myAlliance.name);
    setNewDesc(myAlliance.description || '');
    
    // parse crest: "pattern|colorA-colorB|emoji"
    const crestParts = myAlliance.crest_emoji.split('|');
    if (crestParts.length === 3) {
      setNewPattern(crestParts[0]);
      const colors = crestParts[1].split('-');
      if (colors.length === 2) {
        setNewColorA(colors[0]);
        setNewColorB(colors[1]);
      }
      setNewCharge(crestParts[2]);
    } else {
      setNewPattern('solid');
      setNewColorA('sable');
      setNewColorB('or');
  ...
  const handleUpdateAlliance = async () => {
    ...
    try {
      const computedCrest = `${newPattern}|${newColorA}-${newColorB}|${newCharge}`;
      const { data, error } = await (supabase as any).rpc('update_alliance', {
        p_alliance_id: myAlliance.alliance_id,
        p_name: newName,
        p_description: newDesc,
        p_crest_emoji: computedCrest,
        p_user_id: userId
      ...
      if (error) {
        ...
        toast({ title: "Kingdom Updated!", description: `The Dynasty has been modernized.` });
  ...
  const handleSendChatMessage = async (e: React.FormEvent) => {
    ...
    haptics('light');
    try {
      const { data, error } = await (supabase as any).from('alliance_chat').insert({
      ...
      console.error(err);
      toast({ title: "Failed to pin message", description: err.message || "Unknown error", variant: "destructive" });
    } finally {
      setSendingMsg(false);
    }
  };
              ...
              <div className="space-y-8">
                {isEditing ? (
                  ...
                  <div className="panel-3d bg-white border-2 border-primary/20 rounded-3xl p-6 shadow-md text-foreground relative overflow-hidden">
                    ...
                    {/* Dangerous buttons */}
                    <div className="mt-8 flex justify-between relative z-10 border-t-2 border-slate-100 pt-4">
                      {myRole === 'king' ? (
                        <Button 
                          variant="outline" 
                          size="sm" 
                          onClick={handleEditClick} 
                          className="text-primary hover:bg-primary/5 text-[11px] font-black uppercase flex items-center gap-1.5 transition-colors rounded-xl px-4 py-2"
                        >
                          <Info className="w-4 h-4" /> Edit Kingdom
                        </Button>
                      ) : (
                        <div />
                      )}
