...
BEGIN
  ...
  IF v_caller IS NULL OR user_uuid IS NULL OR user_uuid <> v_caller THEN
  ...
  SELECT COALESCE(stars, 0) INTO v_stars FROM public.profiles WHERE id = user_uuid FOR UPDATE;
  IF v_stars < req_stars THEN
  ...
  IF req_stars > 0 THEN
    ...
    UPDATE public.profiles SET stars = GREATEST(COALESCE(stars, 0) - req_stars, 0) WHERE id = user_uuid;
...
REVOKE EXECUTE ON FUNCTION public.upgrade_character(text, text) FROM anon;
